...
1. On the **Configuration** tab, enter the following configuration select **Tags** and click on **Review + Create** then **Create**:
   ...
   - Private DNS zone: (default) **privatelink.database.windows.net**
    ...
    ![Screenshot showing the 'Create a private endpoint' blade, 'Configuration' tab.](images/Exercise3/private-endpoint.png "Create a Private Endpoint - Configuration")
...
5. Open a new browser tab and paste the IP address into the address bar. Verify that the SmartHotel360 application is now available in Azure.
    ...
    > **Note**: 
      ...
      1.  If the browser does not show the application, check if you have provided the correct SQL database connection in the previous task. If yes, then follow the below instructions to access the smarthotel application:
           ...
           1. Under Security, select Firewalls and virtual networks. Set `Deny public network access` to **Unchecked** and `Allow Azure services and resources to access this server` to **Yes**, then Save your changes.

              ![](images/Exercise3/note-task8-denyfirewall.png)
