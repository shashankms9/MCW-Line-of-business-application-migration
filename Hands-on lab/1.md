...
3.  In **1: Generate Azure Migrate project key**, provide below name for the **Azure Migrate appliance** that you will set up for discovery of Hyper-V VMs. Select **Generate key** to start the creation of the required Azure resources. 
    >**Note**: If the Azure Migrate project key generation failed with error Vault name 'SmartHotekMigrxxxx' is already in use. In this case go to the Azure Migrate main page and create a **new project** with different name, select that newly created project and generate the key again.   
...
1. Search for **AzureMigrateWS<inject key="DeploymentID" enableCopy="false" />** Log Analytics workspace under **Azure Migrate** RG.   
